...
class PanelButton extends PanelMenu.Button {
    ...
    /**
     * @private
     * @type {InstanceType<typeof SpectrumVisualizer>}
     */
    menuSpectrum;
    /**
     * @private
    ...
    updateWidgets(flags) {
        ...
        if (flags & WidgetFlags.MENU_IMAGE) {
            if (this.menuImage != null) {
                this.menuBox.remove_child(this.menuImage);
        ...
        if (flags & WidgetFlags.MENU_SPECTRUM) {
            if (this.extension.showSpectrumVisualizer) {
                this.addMenuSpectrum();
            } else if (this.menuSpectrum != null) {
                this.menuSpectrum.stop();
                this.menuBox.remove_child(this.menuSpectrum);
                this.menuSpectrum.destroy();
                this.menuSpectrum = null;
            }
        }
    ...
    async addMenuImage() {
        ...
        if (this.menuImage.get_parent() == null) {
            this.menuBox.add_child(this.menuImage);
    ...
    addMenuSpectrum() {
        const width = this.getMenuItemWidth();
        if (this.menuSpectrum == null) {
            this.menuSpectrum = new SpectrumVisualizer(width);
        } else {
            this.menuSpectrum.setSpectrumWidth(width);
        }
        // Sync animation state with current playback
        if (this.playerProxy.playbackStatus === PlaybackStatus.PLAYING) {
            this.menuSpectrum.start();
        } else if (this.playerProxy.playbackStatus === PlaybackStatus.PAUSED) {
            this.menuSpectrum.pause();
        } else {
            this.menuSpectrum.stop();
        }
        if (this.menuSpectrum.get_parent() == null) {
            this.menuBox.add_child(this.menuSpectrum);
            debugLog("Added menu spectrum visualizer");
        }
    }
    ...
    addMenuControls(flags) {
        if (this.menuControls == null) {
            this.menuControls = new St.BoxLayout({
                styleClass: "popup-menu-controls-box",
                xAlign: Clutter.ActorAlign.CENTER,
                xExpand: true,
            });
        }
        if (flags & WidgetFlags.MENU_CONTROLS_LOOP) {
            const isLoopActive =
                ...
                this.playerProxy.loopStatus === LoopStatus.PLAYLIST;
        ...
        if (flags & WidgetFlags.MENU_CONTROLS_SHUFFLE) {
            const isShuffleActive = !!this.playerProxy.shuffle;
            this.addMenuControlIcon(
    ...
    addMenuControlIcon(options, reactive, onClick, isActive = false) {
        let extraClasses = "";
        if (options.name === "playpausestop") {
            extraClasses += " popup-menu-control-icon-playpause";
        } else if (isActive) {
            extraClasses += " popup-menu-control-icon-active";
        }

        const icon = new St.Icon({
            name: options.name,
            iconName: options.iconName,
            styleClass: `popup-menu-icon popup-menu-control-icon${extraClasses}`,
            trackHover: reactive,
            opacity: reactive ? (isActive ? 255 : 200) : 120,
            reactive,
    ...
    addProxyListeners() {
        this.addProxyListener("Metadata", () => {
            this.updateWidgets(
                WidgetFlags.PANEL_LABEL | WidgetFlags.MENU_LABELS | WidgetFlags.MENU_SLIDER,
            );
        });
        this.addProxyListener("PlaybackStatus", () => {
            ...
            if (this.playerProxy.playbackStatus !== PlaybackStatus.PLAYING) {
                ...
                this.menuSlider?.pauseTransition();
                if (this.menuSpectrum != null) {
                    if (this.playerProxy.playbackStatus === PlaybackStatus.PAUSED) {
                        this.menuSpectrum.pause();
                    } else {
                        this.menuSpectrum.stop();
                    }
